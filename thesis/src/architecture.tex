...
The orchestrator described below is implemented
(\texttt{scripts/run\_pipeline.py}), first validated by a small real pilot
run against real Ollama, PyPI, and Docker (recorded in the accompanying
implementation documentation, \texttt{docs/pipeline-orchestrator.md}), and
since run over the complete 187-row evaluation split, again against real
Ollama, PyPI, and Docker throughout (\Cref{chap:validation}). Each of
ErrorClassifier through ResultLogger exists, is independently tested, and is
called by the orchestrator exactly as described below, including the
bounded two-round feedback mode described later in this section. The
Round-1-versus-final comparison this run produces is reported in
\Cref{chap:validation}; full-scale knowledge-graph population from it
remains future work (\Cref{chap:future-work}). Result logging and
knowledge-graph enrichment were already implemented before the orchestrator
(\Cref{sec:m-resultlogger}) and are unchanged by it; the orchestrator only
calls them.
...
ChatRepair~\cite{xia2024chatrepair}. Rather than running the evaluation split
twice, once per mode, and risking that the language model's non-zero
sampling temperature turns ordinary run-to-run variation into an apparent
Round-2 effect, the evaluation reads both conditions from a single
two-round run: the Round-1 outcome is never re-sampled, only carried
forward or extended by Round 2. \Cref{chap:validation} reports the
resulting Round-1-versus-final comparison.
